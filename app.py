...
def post_report():
    ...
    new_report = ReportMinsMax(errors=errors, days_back=days_back,
                               coins_requested=coins_requested, max=max, min=min, owner_id=owner_id, date=date)
...
def get_reports():
    reports = ReportMinsMax.query.filter_by(
        owner_id=session.get("user_id")).all()
    result = []
    for row in reports:
        result.append({
            'max': row.max,
